...
// Escreve no localStorage sem deixar erro de cota (ou qualquer outro) travar o app
// silenciosamente. Retorna true/false para quem chamou decidir o que fazer.
function safeLocalStorageSet(key, value){
  try{
    localStorage.setItem(key, value);
    return true;
  }catch(e){
    console.error("Falha ao salvar em localStorage:", key, e);
    if(typeof toast === "function"){
      const cheio = e && (e.name === "QuotaExceededError" || e.code === 22);
      toast(cheio
        ? "⚠️ Armazenamento cheio — apague avaliações antigas ou fotos para liberar espaço."
        : "⚠️ Não foi possível salvar os dados localmente.");
    }
    return false;
  }
}
...
function saveAssessments(list){
  return safeLocalStorageSet(STORE_KEY, JSON.stringify(list));
}
function loadSettings(){
  try{ return JSON.parse(localStorage.getItem(SETTINGS_KEY)) || { aiEndpoint:"", thresholds:{} }; }
  catch(e){ return { aiEndpoint:"", thresholds:{} }; }
}
function saveSettingsToStore(s){ return safeLocalStorageSet(SETTINGS_KEY, JSON.stringify(s)); }
...
function savePestPhotosToStore(p){ return safeLocalStorageSet(PEST_PHOTOS_KEY, JSON.stringify(p)); }

// Escapa texto digitado pelo usuário antes de inserir via innerHTML (evita XSS
// em campos livres como talhão/fazenda/responsável).
...
// Gera pontos a partir do 1º ponto marcado (âncora com GPS real). Os pontos gerados
// ficam marcados como "sugerido" (posição calculada, não GPS real) até que a pessoa
// abra o ponto no local e capture a localização real — savePoint() então remove a marca.
...
function squareBBoxAroundPoint(lat, lng, radiusM){
  return {
    minLat: lat - metersToDegLat(radiusM),
    maxLat: lat + metersToDegLat(radiusM),
    minLng: lng - metersToDegLng(radiusM, lat),
    maxLng: lng + metersToDegLng(radiusM, lat)
  };
}

// Caminho em "W" (zigue-zague clássico de amostragem) com N vértices normalizados
// em [0,1]x[0,1], depois deslocado para que o 1º vértice caia exatamente na âncora.
function gerarPontosZigzag(nTotal, bbox, anchor){
  const baseLegs = [[0,0],[0.25,1],[0.5,0],[0.75,1],[1,0]];
  let norm;
  if(nTotal<=baseLegs.length){
    norm = baseLegs.slice(0,nTotal);
  } else {
    norm = [];
    for(let i=0;i<nTotal;i++){
      norm.push([ i/(nTotal-1), (i%2===0)?0:1 ]);
    }
  }
  const raw = norm.map(([nx,ny])=>[
    bbox.minLat + ny*(bbox.maxLat-bbox.minLat),
    bbox.minLng + nx*(bbox.maxLng-bbox.minLng)
  ]);
  const dLat = anchor.lat - raw[0][0], dLng = anchor.lng - raw[0][1];
  return raw.map(([lat,lng])=>[lat+dLat, lng+dLng]);
}

// Grade regular ancorada no 1º ponto: passos de "spacing" metros a partir da âncora,
// cobrindo o retângulo (bbox do talhão ou quadrado manual em volta da âncora).
function gerarPontosGrade(spacingM, bbox, anchor){
  const dLat = metersToDegLat(spacingM);
  const dLng = metersToDegLng(spacingM, anchor.lat);
  const stepsUp = Math.max(0, Math.ceil((bbox.maxLat-anchor.lat)/dLat));
  const stepsDown = Math.max(0, Math.ceil((anchor.lat-bbox.minLat)/dLat));
  const stepsRight = Math.max(0, Math.ceil((bbox.maxLng-anchor.lng)/dLng));
  const stepsLeft = Math.max(0, Math.ceil((anchor.lng-bbox.minLng)/dLng));
  const pts = [];
  for(let i=-stepsDown; i<=stepsUp; i++){
    for(let j=-stepsLeft; j<=stepsRight; j++){
      pts.push([anchor.lat + i*dLat, anchor.lng + j*dLng]);
    }
  }
  return pts;
}

// N pontos igualmente espaçados em círculo ao redor da âncora, a uma distância fixa.
function gerarPontosRadial(nOutros, radiusM, anchor){
  const pts = [];
  for(let k=0;k<nOutros;k++){
    const angle = (2*Math.PI*k)/nOutros;
    const dLat = metersToDegLat(radiusM)*Math.cos(angle);
    const dLng = metersToDegLng(radiusM, anchor.lat)*Math.sin(angle);
    pts.push([anchor.lat+dLat, anchor.lng+dLng]);
  }
  return pts;
}

function talhaoPoligono(){
  if(draft.kml && draft.kml.polygons && draft.kml.polygons.length) return draft.kml.polygons[0].coords;
  return null;
}

function abrirGerarPontos(){
  if(!draft.pontos.length || draft.pontos[0].lat==null){
    toast("Marque o 1º ponto (com GPS) antes de gerar os demais");
    return;
  }
  const poly = talhaoPoligono();
  const baseSel = document.getElementById("gp-base");
  baseSel.querySelector('option[value="talhao"]').disabled = !poly;
  baseSel.value = poly ? "talhao" : "manual";
  document.getElementById("gp-padrao").value = "zigzag";
  atualizarCamposGerarPontos();
  document.getElementById("modal-gerar-pontos").classList.remove("hidden");
}

function atualizarCamposGerarPontos(){
  const padrao = document.getElementById("gp-padrao").value;
  const base = document.getElementById("gp-base").value;
  const poly = talhaoPoligono();
  document.getElementById("gp-base-info").textContent = poly
    ? "Contorno importado disponível — pode usar o talhão ou informar uma área manual."
    : "Nenhum talhão (KML) importado nesta avaliação — os pontos serão calculados a partir do raio informado.";
  document.getElementById("gp-campo-raio").style.display = (base==="manual") ? "block" : "none";
  document.getElementById("gp-campo-qtd").style.display = (padrao!=="grade") ? "block" : "none";
  document.getElementById("gp-campo-espacamento").style.display = (padrao==="grade") ? "block" : "none";
}

function confirmarGerarPontos(){
  const anchor = { lat: draft.pontos[0].lat, lng: draft.pontos[0].lng };
  const padrao = document.getElementById("gp-padrao").value;
  const base = document.getElementById("gp-base").value;
  const poly = (base==="talhao") ? talhaoPoligono() : null;
  if(base==="talhao" && !poly){ toast("Nenhum talhão importado — escolha a opção de área manual"); return; }

  const raioM = parseFloat(document.getElementById("gp-raio").value) || 100;
  const bbox = poly ? polygonBBox(poly) : squareBBoxAroundPoint(anchor.lat, anchor.lng, raioM);

  let novos = [];
  if(padrao==="zigzag"){
    const qtd = Math.max(1, parseInt(document.getElementById("gp-qtd").value) || 4) + 1; // +1 = inclui a âncora no cálculo do caminho
    novos = gerarPontosZigzag(qtd, bbox, anchor).slice(1); // remove a âncora (já existe como ponto 1)
  } else if(padrao==="grade"){
    const spacing = parseFloat(document.getElementById("gp-espacamento").value) || 80;
    novos = gerarPontosGrade(spacing, bbox, anchor)
      .filter(([lat,lng])=> Math.abs(lat-anchor.lat)>1e-7 || Math.abs(lng-anchor.lng)>1e-7); // remove a âncora
  } else {
    const qtd = Math.max(1, parseInt(document.getElementById("gp-qtd").value) || 4);
    novos = gerarPontosRadial(qtd, raioM, anchor);
  }

  let foraDoTalhao = 0;
  if(poly){
    const dentro = novos.filter(([lat,lng])=>pointInPolygon(lat,lng,poly));
    foraDoTalhao = novos.length - dentro.length;
    novos = dentro;
  }

  if(novos.length===0){
    toast("Nenhum ponto pôde ser gerado com esses parâmetros — tente aumentar o raio/espaçamento");
    return;
  }

  novos.forEach(([lat,lng])=>{
    draft.pontos.push({ lat, lng, photo:null, pestId:null, count:null, estagios:null, sugerido:true, timestamp:new Date().toISOString() });
  });
  closeModal("modal-gerar-pontos");
  renderColeta();
  toast(novos.length+" ponto(s) gerado(s)"+(foraDoTalhao ? " · "+foraDoTalhao+" fora do talhão foram descartados" : ""));
}
...
function renderColeta(){
  ...
  draft.pontos.forEach((p,idx)=>{
    ...
    card.onclick = ()=>openEditPoint(idx);
    card.innerHTML = `
      ...
      <div class="point-meta">
        <div class="pname">Ponto ${idx+1}${pest ? " — "+pest.apelido : " — sem infestação"}${p.sugerido ? ` <span class="muted" style="font-weight:400;">(posição estimada — toque p/ capturar GPS real e preencher)</span>` : ""}</div>
      ...
      <button class="icon-btn" style="width:30px;height:30px;font-size:13px;" onclick="event.stopPropagation();removePoint(${idx})">✕</button>
    `;
    list.appendChild(card);

    if(p.lat && coletaMap){
      const marker = L.circleMarker([p.lat,p.lng], {radius:9, color: p.sugerido ? "#9aa68c" : statusColor(status), fillColor: p.sugerido ? "#9aa68c" : statusColor(status), fillOpacity: p.sugerido ? 0.35 : 0.85, weight:2, dashArray: p.sugerido ? "3 3" : null}).addTo(coletaMap);
      marker.bindTooltip("Ponto "+(idx+1)+(p.sugerido ? " (sugerido)" : ""), {permanent:false});
...
// Reabre um ponto já existente (inclusive os gerados automaticamente) para
// capturar o GPS real no local e/ou preencher a praga e a contagem.
function openEditPoint(idx){
  editingPointIndex = idx;
  const p = draft.pontos[idx];
  pointDraft = { lat:p.lat, lng:p.lng, photo:p.photo, pestId:p.pestId, count:p.count, estagios:p.estagios ? {...p.estagios} : null, sugerido:p.sugerido };
  document.getElementById("modal-point-title").textContent = "Editar ponto — #"+(idx+1);
  const statusEl = document.getElementById("gps-status");
  if(p.lat!=null){
    statusEl.textContent = "📍 "+p.lat.toFixed(5)+", "+p.lng.toFixed(5)+(p.sugerido ? " (posição estimada — toque para atualizar com o GPS real)" : " — toque para atualizar");
    statusEl.className = "muted";
  } else {
    statusEl.textContent = "Toque para capturar GPS";
    statusEl.className = "dashline";
  }
  document.getElementById("photo-preview").style.display = p.photo ? "block" : "none";
  if(p.photo) document.getElementById("photo-preview").src = p.photo;
  document.getElementById("photo-input").value = "";
  document.getElementById("ai-suggest-box").innerHTML = "";
  if(p.pestId){
    const pest = getPestById(draft.cultura, p.pestId);
    document.getElementById("point-pest-selected").style.display = "block";
    document.getElementById("sel-pest-thumb").innerHTML = pestVisualInnerHTML(draft.cultura, p.pestId);
    document.getElementById("sel-pest-name").textContent = pest.nome;
    document.getElementById("sel-pest-unit").textContent = "Unidade: "+pest.unidade+" · NC referência: "+effectiveThreshold(draft.cultura,p.pestId);
    renderPointCountFields(pest);
    if(pestHasStages(pest) && p.estagios){
      pest.estagios.forEach(e=>{
        const inp = document.querySelector(`#point-count-fields input[data-stage="${e.id}"]`);
        if(inp) inp.value = p.estagios[e.id] || 0;
      });
    } else if(document.getElementById("point-count")){
      document.getElementById("point-count").value = (p.count!=null) ? p.count : "";
    }
...
function savePoint(){
  ...
  const finalPoint = {...pointDraft, timestamp: new Date().toISOString()};
  delete finalPoint.sugerido; // uma vez salvo/editado, deixa de ser posição estimada
  if(editingPointIndex!=null){
    draft.pontos[editingPointIndex] = finalPoint;
    toast("Ponto "+(editingPointIndex+1)+" atualizado");
  } else {
    draft.pontos.push(finalPoint);
...
function historyCard(a, showDelete){
  ...
  div.innerHTML = `
    <div class="hist-top">
      <div>
        <div style="font-weight:700;font-size:15px;">${escapeHtml(a.talhao)}</div>
        <div class="muted">${escapeHtml(a.fazenda)} · ${new Date(a.data).toLocaleDateString('pt-BR')}</div>
      </div>
...
// jsPDF usa fontes padrão (Helvetica/WinAnsi) que não têm o glifo "≥" — ao tentar
// desenhar esse caractere o PDF corrompe o texto (vira "e1,5cm" em vez de "≥1,5cm")
// e a largura calculada fica errada, fazendo o texto vazar da célula/página.
// Por isso, qualquer texto de praga que vá para o PDF passa por aqui antes.
function pdfSafe(s){
...
async function exportPDFInner(a){
  ...
  if(summary.length===0){
    ...
    const rows = summary.map(s=>{
      ...
      return [
        s.pest.apelido,
        s.media.toFixed(1)+" "+pdfSafe(s.pest.unidade),
        nc!=null ? String(nc) : "—",
        String(s.nPontos),
    ...
    summary.forEach(s=>{
      ...
      if(nc){
        ...
        const idxText = `Índice de infestação: ${idx.toFixed(0)}% do nível de controle (${s.media.toFixed(1)} de ${nc} ${pdfSafe(s.pest.unidade)})`;
      ...
      if(stageEarlyWarning(s.stageMedias)){
        doc.setFont("helvetica","bold"); doc.setFontSize(8.5); doc.setTextColor(168,130,20);
        const warnLines = doc.splitTextToSize("ATENÇÃO: postura/adultos presentes — mesmo sem atingir o NC hoje, é sinal de pressão para os próximos dias. Reavaliar em breve.", pageW-margin*2);
        doc.text(warnLines, margin, y); y += warnLines.length*4 + 2;
      }
