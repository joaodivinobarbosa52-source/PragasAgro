...
function locateNow(map, which, btnEl){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    if(which==="coleta") lastKnownPos = {lat, lng, accuracy};
...
function openAddPoint(){
  ...
  if(lastKnownPos){
    // pré-preenche com a última posição ao vivo do mapa; toque em "Capturar localização" pra atualizar
    pointDraft.lat = lastKnownPos.lat;
    pointDraft.lng = lastKnownPos.lng;
    statusEl.textContent = "📍 "+lastKnownPos.lat.toFixed(5)+", "+lastKnownPos.lng.toFixed(5)+(lastKnownPos.accuracy ? " (±"+Math.round(lastKnownPos.accuracy)+"m)" : "")+" — toque para atualizar";
    statusEl.className = "muted";
  } else {
    statusEl.textContent = "Toque para capturar GPS";
    statusEl.className = "dashline";
  }
  // tenta obter uma leitura fresca em segundo plano, sem travar a abertura do modal
  captureGPS(true);
}

function captureGPS(silent){
  const statusEl = document.getElementById("gps-status");
  if(!navigator.geolocation){ if(!silent) statusEl.textContent = "Geolocalização não suportada"; return; }
  if(!silent) statusEl.textContent = "Capturando...";
  navigator.geolocation.getCurrentPosition(pos=>{
    pointDraft.lat = pos.coords.latitude;
    pointDraft.lng = pos.coords.longitude;
    lastKnownPos = {lat:pointDraft.lat, lng:pointDraft.lng, accuracy:pos.coords.accuracy};
    statusEl.textContent = "📍 "+pointDraft.lat.toFixed(5)+", "+pointDraft.lng.toFixed(5)+" (±"+Math.round(pos.coords.accuracy)+"m)";
    statusEl.className = "muted";
  }, err=>{
    if(!silent) statusEl.textContent = "Não foi possível obter GPS — verifique permissão de localização";
  }, {enableHighAccuracy:true, timeout:silent?8000:10000, maximumAge: silent?4000:0});
}
