...
// Grava no localStorage sem deixar erro de cota (QuotaExceededError) passar em silêncio.
// Retorna true se salvou de fato; false se a gravação falhou (ex: armazenamento cheio).
function safeLocalStorageSet(key, value){
  try{
    localStorage.setItem(key, value);
    return true;
  }catch(e){
    console.error("Falha ao salvar no localStorage ("+key+"):", e);
    return false;
  }
}
...
function saveAssessments(list){
  const ok = safeLocalStorageSet(STORE_KEY, JSON.stringify(list));
  if(!ok){
    alert("⚠️ Não foi possível salvar a avaliação: o armazenamento do navegador está cheio.\n\nApague avaliações antigas (ou fotos) em Início e tente novamente, ou exporte esta avaliação em PDF antes de continuar.");
  }
  return ok;
}
...
let coletaLastPos = null; // { lat, lng, accuracy, ts } — última posição de GPS "ao vivo" recebida na coleta
...
function parseCoordenadasKml(text){
  // 6 casas decimais = ~11cm de precisão, mais que suficiente para uso agrícola,
  // e evita que coordenadas com 12-15 casas (comuns em exportações de KML) inchem o armazenamento.
  ...
  setTimeout(()=>{
    ...
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(pos=>{
        ...
        coletaLastPos = { lat:pos.coords.latitude, lng:pos.coords.longitude, accuracy:pos.coords.accuracy, ts:Date.now() };
...
function openAddPoint(){
  ...
  document.getElementById("gps-status").textContent = "Capturando...";
  ...
  // captura o GPS automaticamente ao abrir o modal, sem depender do técnico lembrar de tocar no botão
  captureGPS();
}
...
function captureGPS(){
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    // se a captura falhar (timeout/permissão), usa a posição "ao vivo" do mapa como respaldo, se for recente
    if(coletaLastPos && (Date.now()-coletaLastPos.ts) < 30000){
      pointDraft.lat = coletaLastPos.lat;
      pointDraft.lng = coletaLastPos.lng;
      statusEl.textContent = "📍 "+pointDraft.lat.toFixed(5)+", "+pointDraft.lng.toFixed(5)+" (posição aproximada do mapa)";
      statusEl.className = "muted";
    } else {
...
function savePoint(){
  if(!pointDraft.lat && coletaLastPos && (Date.now()-coletaLastPos.ts) < 30000){
    pointDraft.lat = coletaLastPos.lat;
    pointDraft.lng = coletaLastPos.lng;
  }
  if(!pointDraft.lat){
...
function finalizeAssessment(){
  ...
  if(!saved){
    // reverte o push: mantém o draft intacto para o usuário não perder os pontos coletados
    assessments.pop();
    return;
  }
...
async function exportPDFInner(a){
  ...
  try{
    if(resMap){
      const canvas = await html2canvas(document.getElementById("res-map"), {useCORS:true, allowTaint:false, backgroundColor:"#dfe6d8", scale:2});
  ...
  for(let i=1;i<=pageCount;i++){
    ...
    doc.text("PragaTrack · AgroBacabal — Desenvolvido por João Divino Barbosa", margin, pageH-6);
