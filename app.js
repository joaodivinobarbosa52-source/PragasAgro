...
function drawAssessmentMapDataURL(a){
  ...
  pts.forEach((p,idx)=>{
    ...
    if(p.pestId){
      const label = String(p.count).padStart(2,'0')+" und";
      ctx.font="bold 10px Arial";
      const lw = ctx.measureText(label).width;
      const lx = x+11, ly = y;
      ctx.fillStyle="rgba(255,255,255,.92)";
      ctx.fillRect(lx-2, ly-7, lw+4, 14);
      ctx.fillStyle=color;
...
async function exportPDFInner(a){
  ...
  try{
    if(resMap){
      const scale = 2;
      const canvas = await html2canvas(document.getElementById("res-map"), {useCORS:true, allowTaint:false, backgroundColor:"#dfe6d8", scale});
      const ctx = canvas.getContext("2d");
      ...
      pts.forEach((p,idx)=>{
        const status = p.pestId ? statusForCount(a.cultura,p.pestId,p.count) : "ok";
        const color = statusColor(status);
        const pt = resMap.latLngToContainerPoint([p.lat,p.lng]);
        ...
        if(p.pestId){
          ...
          const lw = ctx.measureText(label).width;
          const lx = x+r+3*scale, ly = y;
          ctx.fillStyle = "rgba(255,255,255,.92)";
          ctx.fillRect(lx-2*scale, ly-lf*0.7, lw+4*scale, lf*1.4);
          ctx.fillStyle = color;
          ctx.textAlign="left"; ctx.textBaseline="middle";
          ctx.fillText(label, lx, ly+0.5*scale);
        }
      });
      mapImg = canvas.toDataURL("image/jpeg", 0.9);
  ...
  if(mapImg){
    ...
    doc.text("O número identifica o ponto; o valor ao lado é a contagem observada.", margin, y);
    y += 7;
  }
  ...
  if(summary.length===0){
    ...
    summary.forEach(s=>{
      ...
      if(nc){
        ...
        doc.text(`Índice de infestação: ${idx.toFixed(0)}% do nível de controle (${s.media.toFixed(1)} de ${nc} ${s.pest.unidade})`, margin, y);
        y += 5;
      }
