...
function startNewAssessment(){
  draft = { cultura: homeCrop, fazenda:"", talhao:"", estadio:"", responsavel:"", pontos:[] };
...
function renderPointPestList(){
  ...
  PESTS_DB[draft.cultura]
    ...
    .forEach(p=>{
      ...
      row.innerHTML = `<div><div class="pn">${p.apelido}</div><div class="pl">${p.categoria} · NC ref: ${effectiveThreshold(draft.cultura,p.id)} ${p.unidade}</div></div><span style="color:var(--ink-dim);">›</span>`;
...
function renderGuia(){
  ...
  PESTS_DB[guiaCrop]
    ...
    .forEach(p=>{
      ...
      row.className = "pest-item";
      row.style.cursor = "pointer";
      row.innerHTML = `<div><div class="pn">${p.apelido}</div><div class="pl">${p.nome}</div></div><span style="color:var(--ink-dim);">›</span>`;
