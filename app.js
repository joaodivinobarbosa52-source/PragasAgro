...
// Retorna o conteúdo visual (foto própria salva OU ilustração padrão) de uma praga
function pestVisualInnerHTML(cultura, pestId){
  const custom = getCustomPestPhoto(cultura, pestId);
  return custom
    ? `<img src="${custom}" style="width:100%;height:100%;object-fit:cover;display:block;">`
    : getDefaultPestIcon(pestId);
}
...
function showPestDetail(pest, cultura){
  ...
  document.getElementById("pest-detail-body").innerHTML = `
    ...
    <div class="pest-detail-photo">${pestVisualInnerHTML(cultura, pest.id)}</div>
    <div class="muted" style="text-align:center;font-size:11px;margin:6px 0 10px;">${hasCustom ? "📷 Foto própria salva neste aparelho" : "Ilustração de referência — ainda não é uma foto real"}</div>
    <div class="row" style="margin-bottom:12px;">
      <button class="btn btn-outline btn-sm" onclick="openPestPhotoPicker()">📷 ${hasCustom ? "Trocar foto" : "Adicionar foto real"}</button>
      ${hasCustom ? `<button class="btn btn-outline btn-sm" onclick="resetPestPhoto()">↺ Usar ilustração</button>` : ""}
    </div>
...
function resetPestPhoto(){
  ...
  const pest = getPestById(pestDetailTarget.cultura, pestDetailTarget.pestId);
  showPestDetail(pest, pestDetailTarget.cultura);
  toast("Ilustração padrão restaurada");
}
